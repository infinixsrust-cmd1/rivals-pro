...
function Skin.start(C)
    ...
    task.spawn(function()
        ...
        local ok, err = pcall(function()
                        ...
                        if getnamecallmethod() == "FireServer" and self == eq then
                            ...
                            if C.Skin.Enabled and cname and cname ~= ""
                                ...
                                if data then
                                    ...
                                    C.Skin.Status = "sent " .. tostring(cname) .. " to server"
                                    -- FORWARD (not swallow): Owns* patches say we own it,
                                    -- if server trusts client checks, skin applies for real
                                end
                            end
                        end
                        return old(self, ...)
            ...
            -- VIEWMODEL hook REMOVED (froze the game). Back to data merge only:
            -- Wear stores skin, GetWeaponData serves it. If game snapshot
            -- ignores it, skins stay locker-only. No freeze > no skin.
            -- VIEWMODEL bypass v2: DIRECT paths only (no deep scans).
            -- If this freezes, report and it gets reverted same day.
            pcall(function()
                local LP2 = Common.LP
                C.Skin.Status = "vm hook..."
                ...
                if not ciMod then
                    C.Skin.Status = "unlocked (no vm path)"
                    return
                end
                ...
                if CI and CI._CreateViewModel and not Skin._vmHook then
                    ...
                    C.Skin.Status = "unlocked + vm bypass"
                else
                    C.Skin.Status = "unlocked (vm taken)"
                end
            end)
            if C.Skin.Status == "unlocked: pick skin below" then
                -- keep legacy text if hook block skipped
            end
            Skin.libs = true
        end)
        if not ok then
